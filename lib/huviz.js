...
(function() {
  ...
  Huviz = (function() {
    ...
    Huviz.prototype.truncate_labels_to = 10;
    ...
    Huviz.prototype.find_focused_node_or_edge = function() {
      ...
      if (this.focused_node !== new_focused_node) {
        if (this.focused_node) {
          ...
          this.unscroll_label(this.focused_node);
        }
    ...
    Huviz.prototype.add_quad = function(quad) {
      ...
      if (quad.o.type === RDF_object) {
        ...
        if (subj_n.embryo && is_one_of(pid, NAME_SYNS)) {
          this.set_node_label(subj_n, quad.o.value.replace(/^\s+|\s+$/g, ''));
          this.develop(subj_n);
        }
      }
    ...
    Huviz.prototype.set_node_label = function(node, full_name) {
      var len;
      ...
      if (node.name !== node.full_name) {
        return node.label_truncated_to = this.truncate_labels_to;
      } else {
        return node.label_truncated_to = 0;
      }
    };

    Huviz.prototype.scroll_label = function(node) {
      var wrapped;
      if (!this.truncate_labels_to) {
        return;
      }
      if (node.label_truncated_to) {
        if (!node.scroll_offset) {
          ...
          node.scroll_offset += 1;
          if (node.scroll_offset > this.truncate_labels_to) {
            node.scroll_offset = 0;
          }
        }
        wrapped = node.full_name + "   " + node.full_name + "   " + node.full_name;
        node.name = wrapped.substr(node.scroll_offset, this.truncate_labels_to);
      }
      if (node.name.length < this.truncate_labels_to) {
        console.log("'" + node.name + "' is shorter than " + this.truncate_labels_to);
      }
      if (node.name.length > this.truncate_labels_to) {
        console.debug("'" + node.name + "' is longer than " + this.truncate_labels_to);
      }
    };

    Huviz.prototype.unscroll_label = function(node) {
      return this.set_node_label(node, node.full_name);
    };
